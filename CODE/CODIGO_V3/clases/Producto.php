...
class Producto {
    ...
    public function edit($datos) {
        ...
        $stock = intval($c->test_input($datos[4])); // Convertir a número entero
        ...
        $sql = "UPDATE productos SET nombre = '$nombre', precio_compra = '$pc', precio_venta = '$pv', stock = '$stock', id_proveedor = '$proveedor', id_categoria = '$categoria' WHERE id_producto = $id";
        $result = mysqli_query($conexion, $sql);
        return $result;
    }
    ...
    public function mostrar() {
        $c = new Conexion();
        $conexion = $c->conectar();
        $sql = "SELECT pro.id_producto, pro.nombre, pro.precio_compra, pro.precio_venta, pro.stock, pr.nombre AS id_proveedor, ca.nombre AS id_categoria FROM categorias AS ca INNER JOIN productos AS pro ON pro.id_categoria = ca.id_categoria INNER JOIN proveedores AS pr ON pr.id_proveedor = pro.id_proveedor WHERE pro.estado = 'activo'";
        $result = mysqli_query($conexion, $sql);
        return $result; 
    }
